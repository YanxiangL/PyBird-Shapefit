...
class EisensteinHu(object):
    ...
    def __init__(self, cosmo):
        ...
        self.sound_horizon = self.h * 44.5 * np.log(9.83 / self.Omh2) / np.sqrt(1 + 10 * self.Obh2 ** 0.75)  # in Mpc/h
        self.alpha_gamma = (
    ...
    def __call__(self, kk):
        ...
        return (
            2 * np.pi ** 2 * self.A_s * kk / (100 / 299792.458) ** 4 * (k / 0.05) ** (self.n_s - 1) * (T0 * self.D) ** 2
        )

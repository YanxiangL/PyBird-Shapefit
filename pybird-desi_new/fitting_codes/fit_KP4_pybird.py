...
def lnprior(params, birdmodels):
    ...
    # lower_bounds = [2.8, 0.60, 0.12, 0.020]
    # upper_bounds = [3.3, 0.74, 0.14, 0.024]
    ...
    for i in range(nz):
        ...
        if int(pardict['vary_c4']) == 1:
            # priors += np.where(np.logical_or(c4 < -100.0, c4 > 100.0), -1.0e30, 0.0)
            # priors += -0.5 * c4 ** 2/2.0**2
            # priors += 0.0
...
def lnlike(params, birdmodels, fittingdata, plt):
    ...
    # )
    Pi = birdmodels[model_index].get_Pi_for_marg(
        Ploop, bs[0], shot_noise_ratio, fittingdata.data["x_data"][model_index]
    )
...
if __name__ == "__main__":
    ...
    # start = np.array([   3.05008519e+00,  6.77675261e-01,  1.21569199e-01,  2.23557564e-02,
    #     1.98873273e+00,  1.71635995e+00, -5.92750707e-02,  8.65348045e+00,
    #     -1.54077436e+01,  1.57665786e+02, -1.92345927e+02,  2.12782251e-01,
    #     3.01224064e+00, -7.09279600e+00])
    ...
    # print(lnlike(start, birdmodels, fittingdata, plt))

    # Does an MCMC
    # do_emcee(lnpost, start)
    # do_zeus(lnpost, start)
    # do_dynesty(lnlike, lnprior_transform, start, jobid)
    print(lnlike(np.array([3.00898406, 0.67587391, 0.12082079, 0.02228337, 2.03935988,
           1.79887585, 7.76322793]), birdmodels, fittingdata, plt))

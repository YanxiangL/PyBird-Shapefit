...
def read_chain(pardict, Shapefit, z_bin):
    ...
    if resum == True:
        ...
        if Shapefit:
            fmt_str = (
                "%s_%s_%2dhex%2d_%s_%s_%s_Shapefit_mock_%s_bin_"+str(z_bin)+".hdf5"
                if pardict["do_corr"]
                else "%s_%s_%3.2lfhex%3.2lf_%s_%s_%s_Shapefit_mock_%s_bin_"+str(z_bin)+".hdf5"
            )
        else:
            fmt_str = (
                "%s_%s_%2dhex%2d_%s_%s_%s_Templatefit_mock_%s_bin_"+str(z_bin)+".hdf5"
                if pardict["do_corr"]
                else "%s_%s_%3.2lfhex%3.2lf_%s_%s_%s_Templatefit_mock_%s_bin_"+str(z_bin)+".hdf5"
            )
...
if __name__ == "__main__":
    ...
    for i in range(nz):
        ...
        if FullShape == True:
            ...
            fitcov = np.linalg.inv(np.cov(useful, rowvar=False))
        else:
            fitdata, fitcov = read_chain(pardict, Shapefit, i)

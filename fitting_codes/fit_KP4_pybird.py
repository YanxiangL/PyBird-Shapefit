...
def lnprior(params, birdmodels):
    ...
    # lower_bounds = [2.5, 0.62, 0.08, 0.021]
    # upper_bounds = [3.5, 0.72, 0.16, 0.024]
...
def lnlike(params, birdmodels, fittingdata, plt):
    ...
    for i in range(nz):
        ...
        # np.save('Plin_ELG_FS.npy', Plin)
        # np.save('Ploop_ELG_FS.npy', Ploop)
        ...
        # # print(np.shape(P_model_lin), np.shape(P_model_loop), np.shape(P_model_interp))
        # if birdmodels[i].corr_convert == True:
        #     P_model = P_model_lin + P_model_loop
            
        #     xi, xi_loop = birdmodels[i].pk2xi_fun(P_model, Ploop, bs[0], fittingdata.data["x_data"][i])
            
        #     # print(np.shape(xi))
        #     # np.save('xi_test.npy', xi)
            
        #     Ploop = np.transpose(np.array([xi_loop]), axes=[1, 2, 3, 0])
            
        #     P_model_interp = np.array([np.concatenate((xi[0], xi[1]))]).T
        
        # print(np.shape(Ploop), np.shape(P_model_interp))
        
        # np.save('Pmodel_long_dash.npy', P_model_interp)
            ...
            for i in range(nz):
                ...
                bs = np.array(
                    [
                        ...
                        bs_analytic[7 * i + 2],
                        0.0,
                        bs_analytic[7 * i + 3] * shot_noise_ratio,
                        bs_analytic[7 * i + 4] * shot_noise_ratio,
                        bs_analytic[7 * i + 5] * shot_noise_ratio,
                        # bs_analytic[7 * i + 3],
                        # bs_analytic[7 * i + 4] * float(fittingdata.data["shot_noise"][i]),
    ...
    # chi_squared = np.where(chi_squared > 0.0, chi_squared, 1e30)
...
# c = ChainConsumer()
# chainfile = '../../data/DESI_KP4_LRG_fix_single_pk_0.14hex0.14_3order_nohex_marg_Shapefit_mock_mean_Gaussian_bin_0.hdf5'
# sample_new, log_likelihood_new, max_log_likelihood_new = read_chain_backend(chainfile)
# c.add_chain(sample_new[:, :4], parameters = [r"$\alpha_{\perp}$", r"$\alpha_{\parallel}$", r"$f\sigma_8$", r"$m$"], name = r'$k_{\mathrm{max}} = 0.14 h \mathrm{Mpc}^{-1}$')
# chainfile = '../../data/DESI_KP4_LRG_fix_single_pk_0.16hex0.16_3order_nohex_marg_Shapefit_mock_mean_Gaussian_bin_0.hdf5'
# sample_new, log_likelihood_new, max_log_likelihood_new = read_chain_backend(chainfile)
# c.add_chain(sample_new[:, :4], parameters = [r"$\alpha_{\perp}$", r"$\alpha_{\parallel}$", r"$f\sigma_8$", r"$m$"], name = r'$k_{\mathrm{max}} = 0.16 h \mathrm{Mpc}^{-1}$')
# chainfile = '../../data/DESI_KP4_LRG_fix_single_pk_0.18hex0.18_3order_nohex_marg_Shapefit_mock_mean_Gaussian_bin_0.hdf5'
# sample_new, log_likelihood_new, max_log_likelihood_new = read_chain_backend(chainfile)
# c.add_chain(sample_new[:, :4], parameters = [r"$\alpha_{\perp}$", r"$\alpha_{\parallel}$", r"$f\sigma_8$", r"$m$"], name = r'$k_{\mathrm{max}} = 0.18 h \mathrm{Mpc}^{-1}$')
# chainfile = '../../data/DESI_KP4_LRG_fix_single_pk_0.20hex0.20_3order_nohex_marg_Shapefit_mock_mean_Gaussian_bin_0.hdf5'
# sample_new, log_likelihood_new, max_log_likelihood_new = read_chain_backend(chainfile)
# c.add_chain(sample_new[:, :4], parameters = [r"$\alpha_{\perp}$", r"$\alpha_{\parallel}$", r"$f\sigma_8$", r"$m$"], name = r'$k_{\mathrm{max}} = 0.20 h \mathrm{Mpc}^{-1}$')
# chainfile = '../../data/DESI_KP4_LRG_fix_single_pk_0.22hex0.22_3order_nohex_marg_Shapefit_mock_mean_Gaussian_bin_0.hdf5'
# sample_new, log_likelihood_new, max_log_likelihood_new = read_chain_backend(chainfile)
# c.add_chain(sample_new[:, :4], parameters = [r"$\alpha_{\perp}$", r"$\alpha_{\parallel}$", r"$f\sigma_8$", r"$m$"], name = r'$k_{\mathrm{max}} = 0.22 h \mathrm{Mpc}^{-1}$')
# chainfile = '../../data/DESI_KP4_LRG_fix_single_pk_0.24hex0.24_3order_nohex_marg_Shapefit_mock_mean_Gaussian_bin_0.hdf5'
# sample_new, log_likelihood_new, max_log_likelihood_new = read_chain_backend(chainfile)
# c.add_chain(sample_new[:, :4], parameters = [r"$\alpha_{\perp}$", r"$\alpha_{\parallel}$", r"$f\sigma_8$", r"$m$"], name = r'$k_{\mathrm{max}} = 0.24 h \mathrm{Mpc}^{-1}$')
# chainfile = '../../data/DESI_KP4_LRG_fix_single_pk_0.26hex0.26_3order_nohex_marg_Shapefit_mock_mean_Gaussian_bin_0.hdf5'
# sample_new, log_likelihood_new, max_log_likelihood_new = read_chain_backend(chainfile)
# c.add_chain(sample_new[:, :4], parameters = [r"$\alpha_{\perp}$", r"$\alpha_{\parallel}$", r"$f\sigma_8$", r"$m$"], name = r'$k_{\mathrm{max}} = 0.26 h \mathrm{Mpc}^{-1}$')
# chainfile = '../../data/DESI_KP4_LRG_fix_single_pk_0.28hex0.28_3order_nohex_marg_Shapefit_mock_mean_Gaussian_bin_0.hdf5'
# sample_new, log_likelihood_new, max_log_likelihood_new = read_chain_backend(chainfile)
# c.add_chain(sample_new[:, :4], parameters = [r"$\alpha_{\perp}$", r"$\alpha_{\parallel}$", r"$f\sigma_8$", r"$m$"], name = r'$k_{\mathrm{max}} = 0.28 h \mathrm{Mpc}^{-1}$')
# truth = [1.0, 1.0, 0.45014, 0.0]
# c.configure(legend_color_text=False)
# c.plotter.plot_summary(errorbar=True, truth=truth)
# test = c.analysis.get_latex_table()

# fig, ((ax1, ax2), (ax3, ax4), (ax5, ax6), (ax7, ax8), (ax9, ax10), (ax11, ax12)) = plt.subplots(6, 2, sharex=True)
# unc_LRG = np.sqrt(np.diag(cov_LRG))
# unc_LRG_mean = unc_LRG/5.0
# unc_ELG = np.sqrt(np.diag(cov_ELG))
# unc_ELG_mean = unc_ELG/5.0
# unc_QSO = np.sqrt(np.diag(cov_QSO))
# unc_QSO_mean = unc_QSO/5.0

# ax1.errorbar(k[4:40], k[4:40]*model_LRG[4:40, 1], yerr=k[4:40] * unc_LRG[4:40], fmt = '.', label = 'data', c = 'k', capsize = 1.0, capthick = 0.1, markersize = 1.0, elinewidth = 0.5)
# ax1.plot(k[4:40], k[4:40]*FS_LRG[4:40, 0], label = 'Full-Shape', c = 'b', linewidth = 1.0)
# ax1.plot(k[4:40], k[4:40]*SF_LRG[4:40, 0], label = 'Shapefit', c = 'r', linewidth = 1.0)
# ax1.set_ylabel('LRG ' + r'$kP_0(k)$', fontsize = 4)
# ax1.tick_params(axis='x', labelsize=4)
# ax1.tick_params(axis='y', labelsize=4)
# ax1.legend(fontsize = 4)

# ax2.fill_between(k[4:40], k[4:40] * unc_LRG[4:40], -k[4:40] * unc_LRG[4:40], c = 'k', alpha = 0.5)
# ax2.plot(k[4:40], k[4:40]*(FS_LRG[4:40, 0]-model_LRG[4:40, 1]), label = 'Full-Shape', c = 'b', linewidth = 1.0)
# ax2.plot(k[4:40], k[4:40]*(SF_LRG[4:40, 0]-model_LRG[4:40, 1]), label = 'Shapefit', c = 'r', linewidth = 1.0)
# ax2.set_ylabel('LRG ' + r'$k \Delta P_0(k)$', fontsize = 4)
